...
(https://github.com/IoP/bips/blob/master/bip-0130.mediawiki)
...
(https://github.com/IoP/bips/blob/master/bip-0125.mediawiki).
...
This allows running IoPd without having to do any manual configuration.
...
IoPd can now (optionally) asynchronously notify clients through a
...
`IoP.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/IoP/IoP/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/IoP/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/IoP/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 28337 to a RPC server bound on localhost on port 7474 do:

    stunnel -d 28337 -r 127.0.0.1:7474 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /IoPrpc>
        ...
        ProxyPassReverse http://127.0.0.1:8337/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass IoPd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< IoPrpc:<password>
    </Location>
...
Restarting IoPd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of IoP-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new IoP-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include IoP-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix IoP-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of IoP-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override IoPD and IoPCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [IoP-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add IoPD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libIoPconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "IoP" (Ross Nicoll)
