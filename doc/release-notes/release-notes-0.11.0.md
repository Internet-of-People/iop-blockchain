IoP Core version 0.11.0 is now available from:

  <https://IoP.org/bin/IoP-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer IoP/kB fee to be
...
For example, add the following to `IoP.conf`:
...
related to the blockchain in the IoP system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows IoP Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that IoP Core will 
...
For an example script see [IoP-submittx](https://github.com/laanwj/IoP-submittx).
...
- #5334 `cf87536` libIoPconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new IoP-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only IoPAmountField
- #5753 `0f44672` Add IoP logo to about screen
...
- #5809 `46bfbe7` Add IoP-cli man page
...
- #5849 `41259ca` contrib/init/IoPd.openrc: Compatibility with previous OpenRC init script variables
