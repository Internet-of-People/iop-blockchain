...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total IoP balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"IoPaddress\"\n"
            "\nReturn information about the given IoP address.\n"
            "\nArguments:\n"
            "1. \"IoPaddress\"     (string, required) The IoP address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"IoPaddress\", (string) The IoP address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: IoP address and we have full public key:
        CIoPAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are IoP addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) IoP address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CIoPAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"IoPaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"IoPaddress\"  (string, required) The IoP address to use for the signature.\n"
