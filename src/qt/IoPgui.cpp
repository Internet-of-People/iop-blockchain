...
IoPGUI::IoPGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void IoPGUI::setClientModel(ClientModel *clientModel)
{
...
void IoPGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
void IoPGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void IoPGUI::showHelpMessageClicked()
{
...
void IoPGUI::gotoSendCoinsPage(QString addr)
{
...
void IoPGUI::gotoVerifyMessageTab(QString addr)
{
...
void IoPGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to IoP network", "", count));
}
...
#ifdef ENABLE_WALLET
void IoPGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(IoPUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void IoPGUI::subscribeToCoreSignals()
{
...
void IoPGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    Q_FOREACH (const IoPUnits::Unit unit, units)
    {
...
{
    menu = new QMenu();
    Q_FOREACH(IoPUnits::Unit u, IoPUnits::availableUnits())
    {
...
{
    setText(IoPUnits::name(newUnits));
}
