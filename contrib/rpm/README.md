...
reference the release tarball available from https://IoP.org/bin/ and
...
Currently only `Source10` is used, for the example `IoP.conf` file.
...
in the IoP git repository. At present only an SVG version of the IoP
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the IoP
...
needed when 0.12.1 is released, a proper fix is already in the IoP git
master branch.
...
The `IoP.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `IoP-server` package. At this point, most RPM based
...
The `IoP-server` package should have SELinux support. How to properly do
...
[Ringing Liberty](https://www.ringingliberty.com/IoP/). He has been
packaging IoP for Fedora at least since 2012.
