...
###   This script attempts to download the signature file SHA256SUMS.asc from IoP.org
...
RCSUBDIR="test/"
BASEDIR="https://IoP.org/bin/"
VERSIONPREFIX="IoP-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'IoP-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the IoP Core binary release signing key installed?"
   fi
